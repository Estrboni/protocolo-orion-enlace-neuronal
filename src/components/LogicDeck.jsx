...
function PaletteBlock({ block }) {
  ...
  return (
    <div
      ...
      role="button"
      tabIndex={0}
      aria-label={`${block.type} block. ${block.desc}. Costs ${BLOCK_COSTS_MAP[block.type]}KB.`}
      onKeyDown={(e) => e.key === 'Enter' && drag(e)}
      style={{
...
function ProgramBlock({ block, index }) {
  ...
  const handleParamChange = useCallback((key, value, min = 1, max = 10) => {
    ...
    dispatch(updateBlockParam({ id: block.id, key, value: numVal }))
  }, [dispatch, block.id])
  ...
  return (
    <div
      ...
      style={{
        ...
        fontSize: 9,
        borderRadius: 2,
        outline: showError ? `2px solid ${blockColor}` : 'none'
      }}>
      ...
      {block.type === 'MOVE' && (
        <input
          ...
          onChange={e => handleParamChange('steps', e.target.value, 1, 10)}
          aria-label="Movement steps"
      ...
      {block.type === 'ROTATE' && (
        <select
          ...
          style={{ background: '#000', border: '1px solid #003300', color: '#00d4ff', fontSize: 9, fontFamily: 'var(--font-mono)' }}>
      ...
      {block.type === 'IF_SENSOR' && (
        <input
          ...
          onChange={e => handleParamChange('range', e.target.value, 1, 6)}
          aria-label="Sensor detection range"
      ...
      {block.type === 'LOOP_UNTIL' && (
        <select
          ...
          style={{ background: '#000', border: '1px solid #003300', color: '#bf00ff', fontSize: 9, fontFamily: 'var(--font-mono)' }}>
      ...
      {block.type === 'WAIT' && (
        <input
          ...
          onChange={e => handleParamChange('ticks', e.target.value, 1, 5)}
          aria-label="Wait duration in ticks"
...
export default function LogicDeck() {
  ...
  return (
    <div style={{ width: '100%', height: '100%', display: 'flex', flexDirection: 'column', background: '#050f05', border: '1px solid #003300', overflow: 'hidden' }} role="complementary" aria-label="Logic Deck - drag blocks here to program the bot">
      <div className="panel-header" style={{ fontSize: 9 }}>
        LOGIC DECK
        <span style={{ marginLeft: 'auto', color: overflow ? '#ff0040' : '#006622' }} aria-live="polite">
          {memUsed}/{memTotal}KB {overflow && '⚠ OVERFLOW'}
        </span>
      </div>

      {/* Palette */}
      <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 1, padding: 4, borderBottom: '1px solid #002200' }} role="toolbar" aria-label="Block palette">
        {BLOCK_PALETTE.map(b => <PaletteBlock key={b.type} block={b} />)}
      </div>

      {/* Program */}
      <div style={{ flex: 1, overflowY: 'auto', padding: 4 }} role="list" aria-label="Current program blocks">
        {blocks.length === 0 ? (
          <DropZone />
        ) : (
          <>
      ...
      <div style={{ padding: '4px 8px', borderTop: '1px solid #002200', display: 'flex', gap: 6, alignItems: 'center' }}>
        <span style={{ color: '#003300', fontSize: 9, flex: 1 }} aria-live="polite">{blocks.length} opcodes loaded</span>
        <button onClick={() => dispatch(clearProgram())} className="danger" style={{ fontSize: 8, padding: '2px 8px' }} aria-label="Clear all blocks">CLR</button>
      </div>
    </div>
  )
}
